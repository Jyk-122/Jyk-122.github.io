...
(function() {
  ...
  // Banner inline search (homepage)
  var $bannerInput = jQuery('#banner-search-input');
  var $bannerResult = jQuery('#banner-search-result');
  if ($bannerInput.length && $bannerResult.length) {
    var path = CONFIG.search_path || '/local-search.xml';
    localSearchFunc(path, '#banner-search-input', '#banner-search-result');
    // Clear when clicking outside
    jQuery(document).on('click', function(e) {
      if (!jQuery(e.target).closest('.banner-search-wrapper').length) {
        $bannerInput.val('');
        $bannerResult.html('');
      }
    });
